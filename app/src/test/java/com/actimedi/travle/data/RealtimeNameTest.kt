...
class RealtimeNameTest {
    ...
    private val network = SubwayNetwork(
        ...
        stations = listOf(
            // API가 괄호를 붙여 부르는 쪽.
            SubwayStation("군자", 37.55, 127.07, listOf("5호선", "7호선"), realtimeName = "군자(능동)"),
            // 반대로 우리 쪽에만 괄호가 있는 쪽.
            SubwayStation("교대(법원·검찰청)", 37.49, 127.01, listOf("2호선"), realtimeName = "교대"),
    ...
    fun `API가 쓰는 이름으로 바꾼다`() {
        assertEquals("군자(능동)", network.realtimeNameFor("군자"))
        assertEquals("교대", network.realtimeNameFor("교대(법원·검찰청)"))
    }

    @Test
    fun `경로에 저장된 역 이름도 찾아낸다`() {
        // 경로는 '군자역'처럼 적어 두지만 노선망은 '군자'다.
        assertEquals("군자(능동)", network.realtimeNameFor("군자역"))
    }

    @Test
    fun `짝이 없으면 이름을 그대로 둔다`() {
        assertEquals("강남", network.realtimeNameFor("강남"))
        // 노선망에 없는 곳 — 물어보긴 해야 하니 받은 이름을 돌려준다.
        assertEquals("교동마을 LG자이", network.realtimeNameFor("교동마을 LG자이"))
    }
}
