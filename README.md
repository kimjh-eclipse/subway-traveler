...
**1~8호선은 실측값**을 쓴다. 서울교통공사가 공개한 「역간거리 및 소요시간」(공공누리
제1유형)에서 264개 구간의 역간 시간을 뽑아 에셋에 넣었다. 역마다 시간이 다르다 —
1분에서 4분 30초까지 벌어진다.

나머지 노선(9호선·신분당선·수인분당선·공항철도·GTX·인천1/2호선 등)은 공개 데이터가
1~8호선만 있어 **평균 1분 32초/역**으로 메운다. 이 평균도 위 264개 구간에서 뽑은 값이다.
노선을 특정할 수 없으면 직선거리 ÷ 32km/h에 굴곡 보정 1.25를 곱한다.

한 구간이라도 실측이 없으면 `EstimateBasis.STATIONS`로 표시되고, 전부 실측이면
`MEASURED`가 된다. 화면에는 `예상` 배지를 달고, 값이 틀리면 그 구간만 덮어쓸 수 있다.
같은 시간 모델이 길찾기의 비용 함수이기도 하다.

원본 CSV는 `tools/seoul_metro_station_times.csv`에 남겨 두었다.
...
두 상황은 필요한 정보가 다르다. 계획할 때는 저장된 시각이 진실이고, 실제로 움직일 때는
지금 열차가 언제 오는지가 진실이다. 그래서 노선 화면에 **여행 시작 / 여행 종료** 토글을
두고, 모드에 따라 보여주는 것을 바꾼다.

| 모드 | 표시 |
|---|---|
| 계획 보기 (기본) | 저장된 시각만. 네트워크를 부르지 않는다 |
| 여행 중 | 환승 지점마다 **실시간 도착**을 자동으로 펼친다. 체류 카드에도 붙는다 |

모드를 앱이 짐작하지 않는 이유가 있다 — 저장된 경로에는 날짜가 없고 요일과 시각만 있어
오늘이 그 여행 날인지 알 수 없다. 매주 반복하는 경로를 아침에 열어보기만 해도 실시간으로
튀어버리면 계획을 다듬는 데 방해가 된다.

실시간은 서울시 열린데이터광장 API를 쓴다. HTTPS를 제공하지 않아 그 호스트 하나만
`network_security_config`로 평문을 허용했고, 인증키는 `.env`(git 제외)에서 빌드 시점에
주입한다. 하루 1,000건 제한이 있는데 **여행 중에만 부르므로** 계획 중에는 소모되지 않는다.
...
- `MeasuredTimesTest` — 실측이 추정보다 우선하는지, 빠진 구간만 평균으로 메우는지,
  방향이 달라도 같은 값인지
...
- **노선망** — `app/src/main/assets/subway_map.json`은 OpenStreetMap에서 파생했다.
  ...
  소속만 보정해 되살렸다.
- **역간 소요시간** — 서울교통공사 「역간거리 및 소요시간」, **공공누리 제1유형**
  (출처표시, 상업적 이용·변경 가능). 1~8호선 264개 구간.
