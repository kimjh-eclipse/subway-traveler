...
첫 실행에 예시 경로를 하나 넣어 두던 것을 걷어냈다(시안에서 옮겨 온 **누군가의
하루**였고, 하드코딩된 한국어였다). 이제 처음 켜면 `아직 경로가 없습니다`가 나온다.

플레이 심사자도 앱을 열면 그 화면을 본다. 기능을 보이고 싶다면 스토어 스크린샷과
설명이 그 몫을 해야 한다. 노선도 탭은 첫 실행에도 654개 역을 들고 있으므로,
심사 메모에 `하단 노선도 탭을 눌러 보라`고 적어 두면 도움이 된다.
