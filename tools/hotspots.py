...
# 출처에 없는 이름. `성수동 카페거리`·`을지로 골목`처럼 **부르는 이름**이라
# OSM에도 위키데이터에도 항목이 없다. 그렇다고 한국어로 남겨 두면, 한국어를 못
# 읽는 사람에게 보여 주려고 만든 칸에서 정작 한국어가 남는다.
#
# 그래서 손으로 적되 **지어내지는 않는다**. 앞의 지역 이름은 역 이름표
# (`station_names.json`)에서 출처를 거쳐 확인된 한자를 그대로 쓰고, 뒤에 붙는
# 보통명사(거리·골목·지하상가)만 옮겼다. 확인한 표기는 이렇다:
#
#   명동 明洞 · 홍대입구 弘大入口 · 상수 上水 · 성수 聖水 · 강남 江南
#   압구정로데오 狎鴎亭ロデオ/狎鷗亭羅德奧 · 을지로3가 乙支路3街 · 이태원 梨泰院
#
# 중국어는 번체로 적는다 — zhconv가 간체를 만든다. 반대로 하면 번체를 못 되돌린다.
# 출처가 생기면 여기서 지운다.
MANUAL_NAMES = {
    "명동거리": {"j": "明洞通り", "zh": "明洞街"},
    "홍대 걷고싶은거리": {"j": "弘大歩きたい通り", "zh": "弘大步行街"},
    "상수동 카페거리": {"j": "上水洞カフェ通り", "zh": "上水洞咖啡街"},
    "성수동 카페거리": {"j": "聖水洞カフェ通り", "zh": "聖水洞咖啡街"},
    # 건물 이름이 그대로 간판이다. 중국어권 안내도 영문 그대로 쓴다.
    "커먼그라운드": {"j": "コモングラウンド", "zh": "Common Ground"},
    "강남역 지하상가": {"j": "江南駅地下商店街", "zh": "江南站地下商街"},
    "압구정 로데오거리": {"j": "狎鴎亭ロデオ通り", "zh": "狎鷗亭羅德奧街"},
    "을지로 골목": {"j": "乙支路の路地", "zh": "乙支路小巷"},
    "이태원 거리": {"j": "梨泰院通り", "zh": "梨泰院街"},
    # 일본어는 출처에서 왔고 중국어만 비었던 것들. 망원 望遠 · 신촌 新村 확인.
    "망원시장": {"zh": "望遠市場"},
    "신촌 연세로": {"zh": "新村延世路"},
    "익선동 한옥거리": {"zh": "益善洞韓屋街"},
    "연남동 경의선숲길": {"zh": "延南洞京義線林道"},
}
...
def main():
    ...
    # 어디에도 없는 이름만 손으로 적은 것으로 메운다.
    for spot in spots:
        for key, value in MANUAL_NAMES.get(spot["n"], {}).items():
            spot.setdefault(key, value)
